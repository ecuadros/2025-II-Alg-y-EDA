...
size_t binary_search_compare(Container& container, size_t first, size_t last, const ObjType &object, Compare compare)
{
    if (first >= last) return first;
    while (first < last) {
        size_t mid = (first+last)/2;
        auto midObj = (ObjType)container[mid];
        if (!compare(midObj, object) && !compare(object, midObj))
            return mid;
        if (compare(midObj, object))
            first = mid+1;
        else
            last = mid;
    }
    if (!compare((ObjType)container[first], object)) return first;
    return last;
}
...
public:
       ...
       ObjectInfo*     FirstThat(lpfnFirstThat3 lpfn, size_t level, void *pExtra1, void *pExtra2);*/
        ...
        void ForEach(F&& f, size_t level, Args&&... args)
        {
                for (size_t i = 0; i < m_KeyCount; ++i) {
                        if (m_SubPages[i]) m_SubPages[i]->ForEach(std::forward<F>(f), level + 1, std::forward<Args>(args)...);
                        std::invoke(f, m_Keys[i], level, args...);
                }
                if (m_SubPages[m_KeyCount]) m_SubPages[m_KeyCount]->ForEach(std::forward<F>(f), level + 1, std::forward<Args>(args)...);
        }

        template <typename P, typename... Args>
        ObjectInfo* FirstThat(P&& pred, size_t level, Args&&... args)
        {
                ObjectInfo* found = nullptr;
                for (size_t i = 0; i < m_KeyCount; ++i) {
                        if (m_SubPages[i]) {
                                found = m_SubPages[i]->FirstThat(std::forward<P>(pred), level + 1, std::forward<Args>(args)...);
                                if (found) return found;
                        }
                        if (std::invoke(pred, m_Keys[i], level, args...)) return &m_Keys[i];
                }
                if (m_SubPages[m_KeyCount]) return m_SubPages[m_KeyCount]->FirstThat(std::forward<P>(pred), level + 1, std::forward<Args>(args)...);
                return nullptr;
        }
...
bt_ErrorCode CBTreePage<Trait>::Insert(const keyType& key, const ObjIDType ObjID){
       ...
       size_t pos = binary_search_compare(m_Keys, 0, m_KeyCount, key, m_Compare);
...
{
       auto less = typename Trait::Compare{};
       size_t pos = binary_search_compare(m_Keys, 0, m_KeyCount, key, m_Compare);
       //size_t pos = binary_search(m_Keys, 0, m_KeyCount, key);
       auto usedKey = m_Keys[pos].key;
       if( pos >= m_KeyCount )
       {    if( m_SubPages[pos] )
       ...
       if( !m_Compare(usedKey, key) && !m_Compare(key, usedKey) )
       {
               ObjID = m_Keys[pos].ObjID;
               m_Keys[pos].UseCounter++;
               return true;
       }
       if( m_Compare(key, usedKey) )
               if( m_SubPages[pos] )
                       return m_SubPages[pos]->Search(key, ObjID);
       return false;
}
...
{
    ForEach([](ObjectInfo& info, size_t level, std::ostream* pout){
        *pout << std::string(level, '\t') << info.key << "->" << info.ObjID << '\n';
    }, 0, &os);
}
