...
size_t binary_search(Container& container, size_t first, size_t last, ObjType &object, Compare comp = Compare())
{
       ...
       while( first < last )
       {
               ...
               if( !comp(object, container[mid]) && !comp(container[mid], object) ) // object == container[mid]
                       return mid;
               if( comp(container[mid], object) ) // container[mid] < object
                       first = mid+1;
               else
                       last  = mid;
       }
       if( !comp(container[first], object) ) // container[first] >= object
               return first;
       return last;
}
...
// this is the in-memory version of the CBTreePage
{
 ...
 public:
       ...
       // TODO: #8 FirstThat generalized with variadic templates
...
std::istream& CBTreePage<Trait>::ReadStructure(std::istream& is)
{
       ...
       for (size_t i = 0; i <= keyCount; ++i) {
               ...
               if (hasChild) {
                       ...
                       m_SubPages[i]->m_Parent = this;  // Set parent pointer
